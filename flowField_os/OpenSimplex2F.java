...
public class OpenSimplex2F {
	...
	private double noise2_Base(final double xs, final double ys) {
		...
		final int xsb = fastFloor(xs), ysb = fastFloor(ys);
		final double xsi = xs - xsb, ysi = ys - ysb;

		// Index to point list
		final int index = (int) ((ysi - xsi) / 2 + 1) * 3;

		final double ssi = (xsi + ysi) * -0.211324865405187;
		final double xi = xsi + ssi, yi = ysi + ssi;

		// Point contributions
		for (int i = 0; i < 3; i++) {
			final LatticePoint2D c = LOOKUP_2D[index + i];

			final double dx = xi + c.dx, dy = yi + c.dy;
			double attn = 0.5 - dx * dx - dy * dy;
			if (attn <= 0)
				continue;

			final int pxm = (xsb + c.xsv) & PMASK, pym = (ysb + c.ysv) & PMASK;
	...
	public double noise3_Classic(final double x, final double y, final double z) {
		...
		final double xr = r - x, yr = r - y, zr = r - z;
	...
	public double noise3_XYBeforeZ(final double x, final double y, final double z) {
		...
		final double xr = x + s2 - zz, yr = y + s2 - zz;
	...
	private double noise3_BCC(final double xr, final double yr, final double zr) {
		...
		final int xrb = fastFloor(xr), yrb = fastFloor(yr), zrb = fastFloor(zr);
		final double xri = xr - xrb, yri = yr - yrb, zri = zr - zrb;

		// Identify which octant of the cube we're in. This determines which cell
		// in the other cubic lattice we're in, and also narrows down one point on each.
		final int xht = (int) (xri + 0.5), yht = (int) (yri + 0.5), zht = (int) (zri + 0.5);
		...
		while (c != null) {
			final double dxr = xri + c.dxr, dyr = yri + c.dyr, dzr = zri + c.dzr;
			double attn = 0.5 - dxr * dxr - dyr * dyr - dzr * dzr;
			if (attn < 0) {
				c = c.nextOnFailure;
			} else {
				final int pxm = (xrb + c.xrv) & PMASK, pym = (yrb + c.yrv) & PMASK, pzm = (zrb + c.zrv) & PMASK;
	...
	static {
		...
		for (int i = 0; i < 8; i++) {
			...
			final LatticePoint3D c3 = new LatticePoint3D(i1, j1 ^ 1, k1, 0);
			final LatticePoint3D c4 = new LatticePoint3D(i1, j1, k1 ^ 1, 0);

			// Each single step away on the second half-lattice.
			final LatticePoint3D c5 = new LatticePoint3D(i1 + (i2 ^ 1), j1 + j2, k1 + k2, 1);
			final LatticePoint3D c6 = new LatticePoint3D(i1 + i2, j1 + (j2 ^ 1), k1 + k2, 1);
			final LatticePoint3D c7 = new LatticePoint3D(i1 + i2, j1 + j2, k1 + (k2 ^ 1), 1);
	...
	static {
		...
		final Grad2[] grad2 = { new Grad2(0.130526192220052, 0.99144486137381),
